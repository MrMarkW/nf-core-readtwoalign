# mrmarkw/readtwoalign: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/readtwoalign/usage](https://nf-co.re/readtwoalign/usage)
...
```console
nextflow run mrmarkw/readtwoalign --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull mrmarkw/readtwoalign
```
...
First, go to the [mrmarkw/readtwoalign releases page](https://github.com/mrmarkw/readtwoalign/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
